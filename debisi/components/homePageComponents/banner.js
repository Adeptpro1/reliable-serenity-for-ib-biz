...
function HomeBanner() {
  ...
  return (
    <div className="relative max-w-7xl overflow-hidden rounded-2xl shadow-lg" style={{ marginTop: '32px', marginBottom: '32px', marginRight: 'auto', marginLeft: 'auto', width: '90%' }}>
      <div className="flex transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${current * 100}%)`, width: `${banners.length * 100}%` }}>
        {banners.map((banner) => (
          <a
            key={banner.id}
            href={banner.url || '#'}
          ...
          >
            <img src={banner.image} alt={banner.title} className="w-full h-[120px] sm:h-[200px] md:h-[300px] object-cover" />
