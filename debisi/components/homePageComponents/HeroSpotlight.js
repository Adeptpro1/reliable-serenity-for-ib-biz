...
export default function HeroSpotlight({
  ...
  const startLoop = useCallback(() => {
    const loop = () => {
      ...
      if (isOver.current) setMask(pos.current.x, pos.current.y, spotlightRadius);
  ...
  const handleMouseLeave = useCallback(() => {
    ...
    if (bgRef.current) {
      bgRef.current.style.transition = 'mask-image 0.8s ease-out, -webkit-mask-image 0.8s ease-out';
    }
