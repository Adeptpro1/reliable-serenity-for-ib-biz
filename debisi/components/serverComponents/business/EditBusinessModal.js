...
import { UPDATE_BUSINESS } from "@/graphql/mutations/business/business";
import { UPLOAD_IMAGE, DELETE_IMAGE } from "@/graphql/mutations/common";
...
const UpdateBusiness = ({ business, onClose, onSuccess }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const formData = {
      ...
      followLeadFields: business?.isVerified ? [
        data.collectEmail && "email",
        data.collectPhone && "phone"
      ].filter(Boolean) : null,
    };
  ...
  return (
    <div className="max-w-4xl" style={{ margin: "0 auto", padding: "1rem" }}>
      ...
      <form onSubmit={handleSubmit(onSubmit, onInvalidSubmit)} className="space-y-6">
          ...
          <input
            ...
            {...register("phone", {
              ...
              validate: (value) => {
                ...
                // Also allow other formats: 10–15 digits, optional leading + or country code
                const cleaned = value.trim();
                if (!/^(\+?\d[\d\s\-()]{8,18}\d)$/.test(cleaned)) {
        ...
        {/* Follower Lead Settings */}
        {business?.isVerified && (
          <div className="form-group" style={{ marginTop: "20px", marginBottom: "20px", padding: "15px", border: "1px solid #e2e8f0", borderRadius: "10px", backgroundColor: "#f8fafc" }}>
            <h4 style={{ fontWeight: "600", fontSize: "14px", color: "#334155", marginBottom: "5px" }}>
              Follower Contact Collection Settings
            </h4>
            <p style={{ fontSize: "12px", color: "#64748b", marginBottom: "12px", lineHeight: "1.4" }}>
              Choose which details to collect from users when they follow your business. Follower details show up in your Leads & Contacts dashboard page.
            </p>
            <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
              <label className="inline-flex items-center">
                <input
                  type="checkbox"
                  className="form-checkbox text-blue-600"
                  defaultChecked={business?.followLeadFields ? business.followLeadFields.includes("email") : true}
                  {...register("collectEmail")}
                />
                <span style={{ marginLeft: "5px", fontSize: "13px", color: "#334155" }}>Collect Email Address</span>
              </label>
              <label className="inline-flex items-center">
                <input
                  type="checkbox"
                  className="form-checkbox text-blue-600"
                  defaultChecked={business?.followLeadFields ? business.followLeadFields.includes("phone") : false}
                  {...register("collectPhone")}
                />
                <span style={{ marginLeft: "5px", fontSize: "13px", color: "#334155" }}>Collect Phone Number</span>
              </label>
            </div>
          </div>
        )}
