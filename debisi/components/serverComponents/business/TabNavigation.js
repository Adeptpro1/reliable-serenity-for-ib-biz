...
export default function TabNavigation({
  ...
  const [businessFilter, setBusinessFilter] = useState(initialBusiness || "");
  ...
  const [visitedTabs, setVisitedTabs] = useState(new Set([initialTab || tabs[0].id]));
  ...
  const filteredBusinesses = useMemo(() => {
    ...
    if (businessFilter) {
      items = items.filter(
        (b) =>
          b.slug?.toLowerCase() === businessFilter.toLowerCase() ||
          b.name?.toLowerCase() === businessFilter.toLowerCase(),
      );
    } else {
      if (searchTerm) {
    ...
    return items;
  }, [businesses, searchTerm, selectedCategory, selectedLocation, businessFilter]);
  ...
  const filteredVideos = useMemo(() => {
    ...
    if (businessFilter) {
      items = items.filter(
        (v) =>
          v.business?.slug?.toLowerCase() === businessFilter.toLowerCase() ||
          v.business?.name?.toLowerCase() === businessFilter.toLowerCase(),
      );
    } else if (searchTerm) {
      const terms = searchTerm
        .split(",")
    ...
    return items;
  }, [videos, searchTerm, businessFilter]);
  ...
  const filteredNotices = useMemo(() => {
    ...
    if (businessFilter) {
      items = items.filter(
        (n) =>
          n.business?.slug?.toLowerCase() === businessFilter.toLowerCase() ||
          n.business?.name?.toLowerCase() === businessFilter.toLowerCase(),
      );
    } else if (searchTerm) {
      const terms = searchTerm
        .split(",")
    ...
    return items;
  }, [notices, searchTerm, businessFilter]);
          ...
          {activeLoading ? (
            ...
            >
              ...
              {activeTab === "Noticeboard" && (
                <>
                  {filteredNotices.length > 0 ? (
                    <div className="notice-masonry">
                      {filteredNotices.map((n) => (
                        <div key={n.id} className="notice-masonry__item">
                          <NoticeCard
                            {...n}
                            message={n.content}
