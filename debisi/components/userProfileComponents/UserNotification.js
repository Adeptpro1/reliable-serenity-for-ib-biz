...
const UserNotification = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center" style={{ marginTop: '10px' }}>
        <div className="animate-pulse text-gray-400 font-medium">Loading notifications...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center" style={{ marginTop: '10px' }}>
        <p className="text-red-500 text-sm">Failed to load notifications: {error.message}</p>
      </div>
    );
  }
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-white rounded-lg transition-colors flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
                style={{ padding: '6px 12px', backgroundColor: 'purple', fontSize: '13px' }}
              >
              ...
              <button
                ...
                className="text-white rounded-lg transition-colors flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
                style={{ padding: '6px 12px', backgroundColor: '#D22730', fontSize: '13px' }}
              >
          ...
          <div className="space-y-3">
            {notifications.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-gray-300 text-sm mt-1">No notifications yet.</p>
              </div>
            ) : (
              notifications.map((notification) => (
                <div
                  ...
                  className="bg-white rounded-xl shadow-sm border cursor-pointer hover:shadow-md transition-shadow flex items-start justify-between"
                  style={{
                    padding: '16px',
                    borderColor: notification.isRead ? '#F3F4F6' : '#EFF6FF',
                    borderLeftWidth: notification.isRead ? '1px' : '3px',
                    borderLeftColor: notification.isRead ? '#F3F4F6' : '#3B82F6',
                  }}
                >
                  <div className="flex items-start gap-3 flex-1">
                    <div
                      className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                      style={{ backgroundColor: notification.isRead ? '#F9FAFB' : '#EFF6FF' }}
                    >
                      <FaBell
                        className="w-3 h-3"
                        style={{ color: notification.isRead ? '#9CA3AF' : '#3B82F6' }}
                      />
                    </div>
      ...
      {selectedNotification && (
        <Modal onClose={() => setSelectedNotification(null)}>
          <div style={{ padding: '8px' }}>
            ...
            <p className="text-gray-600 leading-relaxed">{selectedNotification.content}</p>
