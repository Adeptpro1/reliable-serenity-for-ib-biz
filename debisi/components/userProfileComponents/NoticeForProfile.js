...
import {
  ...
  DELETE_NOTICE,
  GET_NOTICE_LEADS,
} from "@/graphql/queries/business/notice";
...
const NoticeForProfile = ({ userData }) => {
  ...
  const townRate = dbPricings.find((p) => p.category === "NOTICE" && p.title.includes("Town"))?.amount ?? 100;
  const cityRate = dbPricings.find((p) => p.category === "NOTICE" && p.title.includes("City"))?.amount ?? 200;
  const stateRate = dbPricings.find((p) => p.category === "NOTICE" && p.title.includes("State"))?.amount ?? 300;

  const dynamicBoostTiers = useMemo(() => [
    {
      id: "TOWN",
      label: "Town Boost",
      emoji: "📍",
    ...
    },
    {
      ...
      costPerDay: stateRate,
      color: "#8b5cf6",
      bg: "rgba(139, 92, 246, 0.08)",
      border: "rgba(139, 92, 246, 0.25)",
    },
  ], [townRate, cityRate, stateRate]);
  ...
  const handleCreateNotice = async (e) => {
    ...
    const COSTS = { TOWN: townRate, CITY: cityRate, STATE: stateRate };
    const tier = noticeForm.boostTier || "TOWN";
    const costPerDay = COSTS[tier] || townRate;
  ...
  const exportLeadsToXLS = async (leads, noticeTitle) => {
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px" }}>
                    ...
                    {noticeForm.boostDays > 0 && (
                      ...
                      >
                        Total: ₦{(noticeForm.boostDays * (dynamicBoostTiers.find(t => t.id === noticeForm.boostTier)?.costPerDay || 100)).toLocaleString()}
                      </span>
                    )}
                  </div>
                  ...
                  {noticeForm.boostDays > 0 && (
                    <div style={{ marginTop: "10px", fontSize: "11px", color: "#64748b", display: "flex", flexDirection: "column", gap: "2px" }}>
                      <span>📡 <strong>Scope:</strong> {dynamicBoostTiers.find(t => t.id === noticeForm.boostTier)?.scope}</span>
                      <span>👥 <strong>Est. Reach:</strong> {dynamicBoostTiers.find(t => t.id === noticeForm.boostTier)?.reach}</span>
                    </div>
                  )}
                </div>
              );
...
const BoostTierModal = ({ walletBalance, onBoost, onClose }) => {
  ...
  const tier = modalBoostTiers.find(t => t.id === selectedTier);
