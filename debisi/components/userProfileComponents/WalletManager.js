  ...
  const handleFundWallet = async () => {
    ...
    try {
      const { data: initData } = await initializeFunding({
        variables: { amount: Number(fundingAmount) }
      });

      const { authorization_url, reference } = initData.initializeWalletFunding;
      window.open(authorization_url, "_blank");
      
      setIsFundingModalOpen(false);
      toast.success("Payment window opened. Click 'Verify' once done.");
    } catch (error) {
