...
const ReachOutModal = ({ userData, onClose }) => {
  ...
  // Cost calculation
  const cost = useMemo(() => {
    const tier1 = dbPricings.find((p) => p.category === "BANNER_ADS" && p.title.includes("Tier 1"))?.amount ?? 3000;
    const tier2 = dbPricings.find((p) => p.category === "BANNER_ADS" && p.title.includes("Tier 2"))?.amount ?? 5000;
    const tier3 = dbPricings.find((p) => p.category === "BANNER_ADS" && p.title.includes("Tier 3"))?.amount ?? 10000;
    if (followerCount <= 1000) return tier1;
    if (followerCount <= 5000) return tier2;
    return tier3;
  }, [followerCount, dbPricings]);
