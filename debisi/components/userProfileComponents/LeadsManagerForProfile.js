...
const LeadsManagerForProfile = ({ userData }) => {
  ...
  const exportToXLSX = async () => {
    ...
    try {
      ...
      const sheetData = leadsToExport.map((l) => {
        let message = "N/A";
        if (typeof l.additionalData === "string") {
          message = l.additionalData;
        } else if (l.additionalData && typeof l.additionalData === "object") {
          message = l.additionalData.comment || l.additionalData.message || l.additionalData.note || JSON.stringify(l.additionalData);
        }
      ...
      // Set friendly column widths for Excel
      ws['!cols'] = [
        { wch: 22 }, // Name
        { wch: 28 }, // Email
        { wch: 18 }, // Phone
        { wch: 35 }, // Message
        { wch: 30 }, // Source Notice
