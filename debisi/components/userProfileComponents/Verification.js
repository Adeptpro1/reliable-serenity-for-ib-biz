...
} from '@/graphql/mutations/business/business';
...
function BusinessVerification({ userData }) {
  ...
  const verEnterprise = dbPricings.find((p) => p.category === "BUSINESS" && (p.title.includes("Enterprise") || p.title.includes("Business Name")))?.amount ?? 1000;
  const verLimited = dbPricings.find((p) => p.category === "BUSINESS" && p.title.includes("Limited"))?.amount ?? 3000;
  const currentVerificationFee = certificationType === 'LIMITED' ? verLimited : verEnterprise;
  ...
  const executeSubmission = async () => {
    ...
    const fee = certificationType === 'LIMITED' ? verLimited : verEnterprise;
            ...
            <div style={{ backgroundColor: "#f8fafc", padding: "16px", borderRadius: "12px", border: "1px solid #e2e8f0", marginBottom: "24px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px", borderTop: "1px dashed #cbd5e1", paddingTop: "12px" }}>
                ...
                <span style={{ fontSize: "16px", color: "#D22730", fontWeight: "800" }}>
                  ₦{currentVerificationFee.toLocaleString()}
                </span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px dashed #cbd5e1", paddingTop: "12px" }}>
                ...
                <span style={{ fontSize: "14px", color: walletBalance >= currentVerificationFee ? "#10b981" : "#ef4444", fontWeight: "700" }}>
                  ₦{walletBalance.toLocaleString()}
                </span>
              </div>
            </div>

            {walletBalance >= currentVerificationFee ? (
              <button
              ...
              >
                {isSubmitting ? "Processing Payment..." : `Pay ₦${currentVerificationFee.toLocaleString()} & Submit`}
              </button>
            ) : (
              <div>
                <p style={{ fontSize: "12px", color: "#ef4444", fontWeight: "600", marginBottom: "12px", textAlign: "center" }}>
                  ⚠️ Insufficient Wallet Balance. You need an additional ₦{(currentVerificationFee - walletBalance).toLocaleString()} to proceed.
                </p>
                <button
                  onClick={() => {
