...
function formatCell(value) {
      ...
      <div className="flex flex-wrap gap-1">
        {value.map((item, idx) => {
          ...
          if (typeof item === 'object' && item !== null) {
            if (item.town || item.city) {
              displayVal = [item.address1, item.town, item.city].filter(Boolean).join(", ");
            } else if (item.url) {
              displayVal = `${item.contactType || 'Link'}: ${item.url}`;
            } else if (item.imageUrl) {
              displayVal = item.imageUrl;
            } else if (item.videoUrl) {
              displayVal = item.videoUrl;
            } else if (item.name) {
              displayVal = item.name;
            } else if (item.title) {
              displayVal = item.title;
            } else {
              try {
  ...
  // Handle objects (like relationships or audit details)
  if (typeof value === 'object' && !(value instanceof Date)) {
    ...
    try {
      ...
      if (keys.length === 0) return <span className="text-gray-300 font-normal">Empty Object</span>;
        ...
        return (
          <div className="flex flex-col gap-1 mt-1 text-[11px]">
            {Object.entries(value).map(([k, v]) => {
              ...
              return (
                <div key={k} className="bg-gray-50 border border-gray-100 rounded px-2 py-0.5 flex items-start gap-1">
                  <span className="font-extrabold text-gray-400 uppercase text-[9px] tracking-wider">{k.replace(/([A-Z])/g, " $1").trim()}:</span>
                  <span className="text-gray-600 font-medium break-all">{formattedVal}</span>
                </div>
              );
            })}
          </div>
      ...
      return (
        <pre className="text-[10px] font-mono bg-gray-50 p-2 rounded-lg border border-gray-100 max-h-40 overflow-y-auto whitespace-pre-wrap break-all text-gray-600">
