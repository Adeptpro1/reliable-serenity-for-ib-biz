...
function MarketplaceClient({ initialProducts = [], highlightId = null }) {
  ...
  useEffect(() => {
    if (highlightId && allProducts?.length > 0) {
      const product = allProducts.find(p => p.id === highlightId);
      if (product) {
  ...
  const handleShareProduct = async (product, e) => {
    ...
    const origin = typeof window !== "undefined" ? window.location.origin : "https://www.debisi.ng";
