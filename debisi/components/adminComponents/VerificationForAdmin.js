            ...
            {selectedRequest?.reviewerNotes && (
              <div className="bg-amber-50 p-4 rounded-xl border border-amber-100">
                <span className="text-[10px] text-amber-700 font-bold uppercase block mb-1">
                  Reviewer Notes {selectedRequest.reviewedAt ? `(${new Date(selectedRequest.reviewedAt).toLocaleDateString()})` : ""}
                </span>
                <p className="text-amber-900 text-sm">{selectedRequest.reviewerNotes}</p>
              </div>
            )}
