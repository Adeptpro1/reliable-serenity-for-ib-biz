...
import { GET_ADMIN_OVERVIEW } from "../../graphql/queries/admin/overview";
...
const AdminOverview = () => {
  ...
  // Derived counts from aggregated adminOverviewStats
  const overviewStats = data?.adminOverviewStats;
  const users         = data?.adminUsersPaginated || [];
  const businesses    = data?.adminBusinessesPaginated || [];
  const therapy       = data?.therapyRequests || [];

  const totalUsers      = overviewStats?.totalUsersCount ?? users.length;
  const totalAdmins     = overviewStats?.totalAdminsCount ?? users.filter(u => u.role === "ADMIN").length;
  const totalBusinesses = overviewStats?.totalBusinessesCount ?? businesses.length;
  const verifiedBiz     = overviewStats?.verifiedBusinessesCount ?? businesses.filter((b) => b.isVerified).length;
  const pendingAds      = overviewStats?.pendingAdsCount ?? 0;
  const totalAds        = overviewStats?.totalAdsCount ?? 0;
  const pendingTherapy  = overviewStats?.pendingTherapyRequestsCount ?? therapy.filter((t) => t.status === "PENDING").length;
  ...
  const stats = [
    {
      ...
      sub: `${overviewStats?.totalTransactionsCount ?? "—"} txns`,
      color: "bg-blue-50",
    },
    {
      title: "Total Users",
  ...
  const totalTx   = overviewStats?.totalTransactionsCount || 0;
  const fundingPct   = totalTx ? Math.round(((overviewStats.fundingCount || 0) / totalTx) * 100) : 0;
  const deductionPct = totalTx ? Math.round(((overviewStats.deductionCount || 0) / totalTx) * 100) : 0;
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between text-sm font-semibold text-gray-700 mb-2">
                  ...
                  <span>{overviewStats?.fundingCount ?? 0}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm font-semibold text-gray-700 mb-2">
                  ...
                  <span>{overviewStats?.deductionCount ?? 0}</span>
                </div>
                <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div
              ...
              <div className="grid grid-cols-3 gap-4 mt-8 pt-6 border-t border-gray-100">
                {[
                  { label: "Total Revenue", value: formatCurrency(overviewStats?.totalRevenue), color: "text-blue-600" },
