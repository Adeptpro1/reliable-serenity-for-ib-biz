...
} from "../../graphql/queries/admin/wallet";
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      const ws = XLSX.utils.json_to_sheet(transactions.map(tx => ({
        ID: tx.id,
        User: `${tx.wallet?.user?.firstName || ""} ${tx.wallet?.user?.lastName || ""}`.trim() || "N/A",
      ...
      <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: "24px" }}>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl font-black text-slate-800">₦{(stats?.totalInternalSpending || 0).toLocaleString()}</h2>
            <div className="mt-4 flex items-center gap-2">
               <span className="text-[10px] font-bold text-rose-500 bg-rose-50 px-2 py-1 rounded-full">{stats?.deductionCount || 0} Service Payments</span>
            </div>
          </div>
        </motion.div>
      </div>
