...
const NoticesForAdmin = () => {
  ...
  useEffect(() => {
    const list = data?.adminAllNotices;
    if (list) {
  ...
  const handleExportCSV = async (noticeId, noticeTitle) => {
    try {
      ...
      // Trigger download with UTF-8 BOM for perfect Excel compatibility
  ...
  const notices = data?.adminAllNotices || [];
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
        ...
        <Table data={filteredNotices} columns={columns} isLoading={loading} onEdit={handleEdit} onDelete={handleDelete} />
      </div>
