...
import FollowingList from '@/components/userProfileComponents/FollowingList'
import { useAuth } from '@/contexts/AuthContext';
import { GET_MY_NOTIFICATIONS } from '@/graphql/queries/notifications';
...
const ProfileButtons = ({ userData }) => {
  ...
  const menuItems = [
    ...
    { icon: <FaHeart className="w-5 h-5 text-rose-500" />, label: 'Following', action: () => { setIsDropdownOpen(false); setShowFollowingModal(true); } },
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      {showFollowingModal && (
        <Modal title="Businesses You Follow" onClose={() => setShowFollowingModal(false)}>
          <FollowingList />
        </Modal>
      )}
    </div>
