...
import { GET_BUSINESS_BY_SLUG } from "@/graphql/queries/business/business";
...
export async function generateMetadata({ params }) {
  ...
  // Use the business logo as the OG image; fall back to the Debisi logo
  const siteUrl = "https://debisi.ng";
  const businessLogo = business?.images?.find((img) => img.isLogo)?.imageUrl;
  const ogImage = businessLogo || `${siteUrl}/debisi_logo.png`;

  return {
    title: `${name} | Debisi NG`,
    description:
      business?.description ||
      `Discover services, products, and contact information for ${name} on Debisi Nigeria.`,
    openGraph: {
...
export default async function BusinessDetailPage({ params }) {
  ...
  const locationString = primaryAddr
    ? `${primaryAddr.town || primaryAddr.city || ""} ${primaryAddr.lg || ""}`.trim() ||
  ...
  return (
    <>
      <DynamicHeader />
      <main
        className="min-h-[80vh] flex items-center justify-center no-limit-view"
        style={{ padding: "20px" }}
      >
        <BusinessView
          isFullPage={true}
          ...
          followLeadFields={business.followLeadFields}
        />
      </main>
      <ScrollFooterWrapper />
      <Footer />
